...
export default function Landing({ centerFormOnLanding }: { centerFormOnLanding: boolean }) {
  ...
  const showAlgomotiveWelcome = !name;

  return (
    <div
      className={`flex h-full transform-gpu flex-col items-center justify-center pb-16 transition-all duration-200 ${centerFormOnLanding ? 'max-h-full sm:max-h-0' : 'max-h-full'} ${getDynamicMargin}`}
    >
      <div ref={contentRef} className="flex max-w-3xl flex-col items-center gap-0 px-4 py-2">
        {showAlgomotiveWelcome && (
          <div className="animate-fadeIn mb-6 flex flex-col items-center">
            <div className="mb-4 flex items-center gap-3">
              <img
                src="/assets/algomotive-mark.svg"
                alt=""
                aria-hidden="true"
                className="h-10 w-10 object-contain"
              />
              <div className="text-sm font-semibold tracking-[0.22em] text-text-secondary">
                ALGOMOTIVE
              </div>
            </div>
            <div className="rounded-full border border-border-medium bg-surface-tertiary/70 px-4 py-1.5 text-xs font-medium tracking-[0.12em] text-text-secondary">
        ...
        >
          ...
          {((isAgent || isAssistant) && name) || name ? (
            ...
            <SplitText
              ...
              text={greetingText}
              className={`${getTextSizeClass(greetingText)} font-semibold tracking-[-0.025em] text-text-primary`}
              delay={50}
              textAlign="center"
              animationFrom={{ opacity: 0, transform: 'translate3d(0,50px,0)' }}
              animationTo={{ opacity: 1, transform: 'translate3d(0,0,0)' }}
        ...
        {showAlgomotiveWelcome && (
          <div className="animate-fadeIn mt-5 max-w-xl text-center text-sm leading-6 text-text-secondary">
            Secure enterprise AI for intelligent workflows, trusted operations, and measurable outcomes.
          </div>
        )}
      </div>
    </div>
